...
(async () => {
  ...
  await page.screenshot({ path: path.join(__dirname, 'admin_moredown_overview_graph.png') });
  console.log('Saved admin_moredown_overview_graph.png');

  // 2. Zoom into Admin block specifically
  await page.evaluate(() => {
    const svg = document.getElementById('campusMapSvg');
    if (svg) {
      svg.setAttribute('viewBox', '140 600 480 300');
    }
  });
  await new Promise(r => setTimeout(r, 600));
  await page.screenshot({ path: path.join(__dirname, 'admin_moredown_zoomed.png') });
  console.log('Saved admin_moredown_zoomed.png');
  ...
  for (const tr of testRoutes) {
    ...
    await page.screenshot({ path: path.join(__dirname, `route_moredown_${safeName}.png`) });
    console.log(`Saved route_moredown_${safeName}.png`);
  }
